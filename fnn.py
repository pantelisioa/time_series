...
# Create a synthetic time series (for example, a sine wave with some noise)
t_vals = np.linspace(0, 20 * np.pi, 2000)
time_series = np.sin(t_vals) + 0.1 * np.random.randn(2000)

# Choose parameters:
m = 10        # Embedding dimension to test
tau = 3       # Time delay
threshold = 10  # Tolerance threshold

fnn_percent = standard_fnn(time_series, m, tau, threshold)
print(f"False Nearest Neighbors for m={m}: {fnn_percent:.2f}%")

# Optionally, one can run the function for various m values and plot FNN% versus m.
m_values = range(1, 16)
fnn_values = [standard_fnn(time_series, m, tau, threshold) for m in m_values]

plt.figure(figsize=(8, 6))
plt.plot(m_values, fnn_values, marker='o')
